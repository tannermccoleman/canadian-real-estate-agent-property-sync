...
add_action('admin_head', array('Creasync_WPAlchemy_MetaBox', '_global_head'));

add_action('admin_footer', array('Creasync_WPAlchemy_MetaBox', '_global_foot'));
...
class Creasync_WPAlchemy_MetaBox
{
	/**
	...
	function Creasync_WPAlchemy_MetaBox($arr)
	{
	...
	function _init()
	{
		...
		if ( ! Creasync_WPAlchemy_MetaBox::_is_post() AND ! Creasync_WPAlchemy_MetaBox::_is_page()) return;
	...
	function _is_post()
	{
		if ('post' == Creasync_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	function _is_page()
	{
		if ('page' == Creasync_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = Creasync_WPAlchemy_MetaBox::_get_current_post_type();
	...
	function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = Creasync_WPAlchemy_MetaBox::_get_post_id();
	...
	function _save($post_id) 
	{
		...
		Creasync_WPAlchemy_MetaBox::clean($new_data);
	...
	function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i])) 
				{
					Creasync_WPAlchemy_MetaBox::clean($arr[$i]);
