...
function aviators_admin_login_head() {
    ...
    if (aviators_settings_get_value('admin', 'login_screen', 'default') != 'on') {
        ...
        body.login #login h1 a {
            background: url('" . $AVIATORS_URL . "/core/plugins/admin/assets/img/admin.png') no-repeat scroll center top transparent;
            height: 241px;
        }
        </style>";
    }
...
function aviators_admin_load_styles() {	
        global $AVIATORS_URL;
    if (is_admin()) {    
        wp_register_style('admin-css', $AVIATORS_URL . '/core/plugins/admin/assets/css/admin.css');
